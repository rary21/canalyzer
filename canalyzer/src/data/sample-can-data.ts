...
export const sampleCANDataSet: CANDataSet = {
  ...
  description: '10秒間のドライビングシナリオ（停車→加速→定速→減速→停車）を模擬したCANデータ'
};
...
export const parseSampleData = () => {
  ...
  console.log(`- 時間範囲: ${sampleCANFrames[0]?.timestamp}ms - ${sampleCANFrames[sampleCANFrames.length - 1]?.timestamp}ms`);
};