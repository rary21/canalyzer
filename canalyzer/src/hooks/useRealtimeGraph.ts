...
export function useRealtimeGraph({ 
  ...
  const updateGraphData = useCallback(() => {
    ...
    const newDataPoints = generateGraphDataPoints(canValues, graphState.selectedSignals)
  ...
  return {
    ...
    // ヘルパー関数
    isDataAvailable: graphState.data.length > 0 && graphState.selectedSignals.length > 0,
