...
export function useWebSocket(config: Partial<WebSocketConfig> = {}): WebSocketHookReturn {
  ...
  }, [finalConfig.maxReconnectAttempts, finalConfig.reconnectInterval, finalConfig.url, handleMessage, startHeartbeat, stopHeartbeat]);
  ...
  }, [finalConfig.url, handleMessage, startHeartbeat, stopHeartbeat, handleReconnect]);
  ...
  // メッセージID購読
  const subscribe = useCallback((messageIds: number[]) => {
    sendMessage({ type: 'subscribe', messageIds });
  }, [sendMessage]);
  
  // メッセージID購読解除
  const unsubscribe = useCallback((messageIds: number[]) => {
