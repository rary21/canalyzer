...
function TestComponent() {
  ...
  return (
    <div>
      ...
      <div data-testid="parse-result">{parseResult ? 'has-result' : 'no-result'}</div>
      <div data-testid="file-name">{fileName || 'no-file'}</div>
      <button onClick={() => setFileName('test.dbc')}>Set File Name</button>
      <button onClick={() => setDBCData({ 
        version: '1.0', 
        nodes: [], 
        messages: new Map() 
      } as DBCDatabase)}>Set DBC Data</button>
      <button onClick={() => setParseResult({ 
        success: true, 
        errors: [], 
        warnings: [] 
      })}>Set Parse Result</button>
    </div>
  )
}
