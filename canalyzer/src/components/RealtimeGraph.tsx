...
export default function RealtimeGraph({
  ...
  const yAxisDomain = useMemo(() => {
    ...
    return ['auto', 'auto']
  }, [chartData, selectedSignals, config.autoScale, config.yAxisMin, config.yAxisMax])
    ...
    return (
      <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
        ...
        {payload.map((entry, index: number) => {
          const signal = selectedSignals.find(s => s.name === entry.dataKey)
          return (
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="p-8 text-center">
          ...
          <p className="text-lg font-medium text-gray-900 mb-2">グラフデータなし</p>
    ...
    <div className="bg-white rounded-lg shadow-sm border">
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">リアルタイムグラフ</h3>
            <p className="text-sm text-gray-600 mt-1">
              過去{config.timeRange / 1000}秒間のデータ（{chartData.length}ポイント）
            </p>
          </div>
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {selectedSignals.map((signal) => {
              ...
              const currentValue = latestDataPoint ? latestDataPoint[signal.name as keyof typeof latestDataPoint] : undefined
