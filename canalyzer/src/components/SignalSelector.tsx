...
// メモ化されたシグナルセレクターコンポーネント
const SignalSelector = React.memo(
  ({
    availableSignals,
    selectedSignals,
    onSelectionChange,
    ...
    // シグナル選択の切り替え（メモ化）
    const toggleSignalSelection = useCallback(
      (signalName: string) => {
        const existingSignal = selectedSignals.find(
          (s) => s.name === signalName
        );
      ...
      [selectedSignals, onSelectionChange, maxSelection, uniqueSignals]
    );
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                グラフに表示するシグナルを選択してください（最大{maxSelection}
                個）
              </p>
            </div>
